...
FROM Products
WHERE change_date <= '2019-08-16') AS subquery
WHERE rn=1)

SELECT p.product_id, COALESCE(new_price,10) AS new_price
FROM pdt_list AS p
LEFT JOIN latest_price AS l
ON p.product_id = l.product_id
GROUP BY p.product_id, new_price
...
FROM (
SELECT p.project_id, p.employee_id, e.name, e.experience_years, DENSE_RANK() OVER (PARTITION BY p.project_id ORDER BY experience_years DESC) AS rank
FROM Project AS p
LEFT JOIN Employee AS e
ON p.employee_id = e.employee_id) AS subquery
WHERE rank=1;
...
SELECT p1_id AS p1, p2_id AS p2, ABS(p2_x_value-p1_x_value)*ABS(p2_y_value-p1_y_value) AS area
FROM (
SELECT p1.id AS p1_id, p1.x_value AS p1_x_value, p1.y_value AS p1_y_value, 
p2.id AS p2_id, p2.x_value AS p2_x_value, p2.y_value AS p2_y_value
FROM Points AS p1
CROSS JOIN Points AS p2) AS s
WHERE NOT p1_x_value=p2_x_value AND NOT p1_y_value=p2_y_value AND p1_id < p2_id;
...
CREATE TYPE action AS ENUM('view', 'like', 'reaction', 'comment', 'report', 'share');
...
FROM (
SELECT action_date, COUNT(remove_date)::NUMERIC/COUNT(action_date) AS percentage
FROM (
SELECT *
FROM Actions AS a
LEFT JOIN Removals AS r
ON a.post_id = r.post_id
WHERE action='report' AND extra='spam') AS s
...
FROM (
SELECT ROW_NUMBER() OVER() AS rn, to_char(visited_on,'YYYY-MM-DD') AS visited_on, SUM(amount) OVER(ORDER BY visited_on ROWS BETWEEN 6 PRECEDING AND CURRENT ROW) AS amount, ROUND(AVG(amount) OVER(ORDER BY visited_on ROWS BETWEEN 6 PRECEDING AND CURRENT ROW),2) AS average_amount
FROM (SELECT visited_on, SUM(amount) AS amount FROM Customer GROUP BY visited_on) AS c
GROUP BY visited_on, amount) AS t
WHERE rn >=7;
...
SELECT gender, to_char(day,'YYYY-MM-DD'), SUM(score_points) OVER (PARTITION BY gender ORDER BY day) AS total
FROM Scores
...
SELECT ROUND(min(sqrt((p2_x-p1_x)*(p2_x-p1_x)+(p2_y-p1_y)*(p2_y-p1_y)))::NUMERIC,2) AS distance
FROM (
SELECT p1.x AS p1_x, p1.y AS p1_y, p2.x AS p2_x, p2.y AS p2_y
FROM plane AS p1
CROSS JOIN plane AS p2) AS s
WHERE NOT (p1_x=p2_x AND p1_y=p2_y);
...
SELECT team_id, team_name, COALESCE(SUM(score),0) AS num_points
FROM (
SELECT match_id, host_team AS team, host_goals AS goals, host_score  AS score
FROM (
SELECT *, 
CASE WHEN host_goals > guest_goals THEN 3 
		  WHEN host_goals = guest_goals THEN 1 
          WHEN host_goals < guest_goals THEN 0 END AS host_score,
CASE WHEN host_goals < guest_goals THEN 3 
		  WHEN host_goals = guest_goals THEN 1 
          WHEN host_goals > guest_goals THEN 0 END AS guest_score
FROM Matches) AS host
UNION
SELECT match_id, guest_team AS team, guest_goals AS goals, guest_score AS score
FROM (
SELECT *, 
CASE WHEN host_goals > guest_goals THEN 3 
		  WHEN host_goals = guest_goals THEN 1 
...
CASE WHEN host_goals < guest_goals THEN 3 
		  WHEN host_goals = guest_goals THEN 1 
          WHEN host_goals > guest_goals THEN 0 END AS guest_score
FROM Matches) AS guest) AS main
RIGHT JOIN  Teams
ON main.team = Teams.team_id
...
WHERE EXTRACT(DAY FROM TO_TIMESTAMP('2019-06-23', 'YYYY-MM-DD')-available_from)>30;
