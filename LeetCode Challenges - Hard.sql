...
CASE WHEN year_start=2018 AND year_start = year_end THEN EXTRACT(DAY FROM period_end) - EXTRACT(DAY FROM period_start::DATE) +1
			WHEN year_start=2018 THEN EXTRACT(DAY FROM referrence_start) -EXTRACT(DAY FROM period_start::DATE) +1 
			WHEN year_end=2018 THEN EXTRACT(DAY FROM period_end) - EXTRACT(DAY FROM referrence_end) +1
			WHEN year_start<2018 AND year_end>2018 AND num_years=2 THEN 365 END AS days_2018,
CASE WHEN year_start=2019 AND year_start = year_end THEN EXTRACT(DAY FROM period_end) - EXTRACT(DAY FROM period_start::DATE) +1
			WHEN year_start=2019 THEN EXTRACT(DAY FROM referrence_start) -EXTRACT(DAY FROM period_start::DATE) +1
			WHEN year_end=2019 THEN EXTRACT(DAY FROM period_end) - EXTRACT(DAY FROM referrence_end) +1
			WHEN year_start<2019 AND year_end>2019 AND num_years=2 THEN 365 END AS days_2019,
CASE WHEN year_start=2020 AND year_start = year_end THEN EXTRACT(DAY FROM period_end) - EXTRACT(DAY FROM period_start::DATE) +1
		   WHEN year_start=2020 THEN EXTRACT(DAY FROM referrence_start) -EXTRACT(DAY FROM period_start::DATE) +1
			WHEN year_end=2020 THEN EXTRACT(DAY FROM period_end) - EXTRACT(DAY FROM referrence_end) +1
			WHEN year_start<2020 AND year_end>2020 AND num_years=2 THEN 365 END AS days_2020            
FROM (
SELECT *, 
  CASE WHEN num_years>0 AND year_start=2018 THEN to_date('2018-12-31','YYYY-MM-DD')
		    WHEN num_years>0 AND year_start=2019 THEN to_date('2019-12-31','YYYY-MM-DD')
            WHEN num_years>0 AND year_start=2020 THEN to_date('2020-12-31','YYYY-MM-DD')  END AS referrence_start,
CASE WHEN num_years>0 AND year_end=2018 THEN to_date('2018-01-01','YYYY-MM-DD') 
		  WHEN num_years>0 AND year_end=2019 THEN to_date('2019-01-01','YYYY-MM-DD') 
           WHEN num_years>0 AND year_end=2020 THEN to_date('2020-01-01','YYYY-MM-DD')  END AS referrence_end                         
FROM (  
SELECT *, EXTRACT(YEAR FROM period_start::DATE) AS year_start, EXTRACT(YEAR FROM period_end) AS year_end, 
EXTRACT(YEAR FROM period_end) - EXTRACT(YEAR FROM period_start::DATE) AS num_years
FROM Sales) AS s) AS t),
sales AS (
SELECT p.product_id, product_name, COALESCE(days_2018*average_daily_sales,0) AS sales_2018, 
COALESCE(days_2019*average_daily_sales,0) AS sales_2019, COALESCE(days_2020*average_daily_sales,0) AS sales_2020
FROM transform AS t
...
SELECT *
FROM (
(SELECT product_id, product_name, CASE WHEN sales_2018>0 then 2018 END AS report_year, sales_2018 AS total_amount
FROM sales)
UNION
(SELECT product_id, product_name, CASE WHEN sales_2019>0 then 2019 END AS report_year, sales_2019 AS total_amount
FROM sales)
UNION
(SELECT product_id, product_name, CASE WHEN sales_2020>0 then 2020 END AS report_year, sales_2020 AS total_amount
FROM sales)) AS combine
